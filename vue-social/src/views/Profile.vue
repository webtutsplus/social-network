    ...
    <div class="col-lg-12">
      <div class="panel profile-cover">
        <div class="profile-cover__img">
          ...
          <h3 v-if="user.name" class="h3">{{user.name}}</h3>
          <h3 v-else class="h3">{{user.email}}</h3>
        </div>
        <div class="profile-cover__action bg--img" data-overlay="0.3">
          <button class="btn btn-rounded btn-info" @click="addFriend()">
      ...
      <div class="panel">
        ...
        <div class="panel-content panel-activity">
          <ul class="panel-activity__list">
            <li v-for="items in posts" :key="items.id">
              <i class="activity__list__icon fa fa-question-circle-o"></i>
              <div v-if="user.name" class="activity__list__header">
                <a href="#">{{user.name}}</a> Posted the Content :
              </div>
              <div v-else class="activity__list__header">
                <a href="#">{{user.email}}</a> Posted the Content :
              </div>
...
export default {
  ...
  methods : {
    getuser(){
      axios.get(API_BASE_URL + "public/user-details/" + this.id,{ "headers" : {
    ...
    getPost() {
      axios.get(API_BASE_URL + "public/"+this.id+"/posts", {"headers" : {
          "Authorization" : "Bearer " + localStorage.getItem("idToken")
        }}).then(resp => {
        if(resp.status === 200){
    ...
    addFriend(){
      axios.get(API_BASE_URL + "private/addFriend/?friendId="+this.id, {"headers" : {
          "Authorization" : "Bearer " + localStorage.getItem("idToken"),
        }}).then(resp => {
          if(resp.status === 200) {
            alert("friend added");
